...
remove_unused_locals (void)
{
  ...
  FOR_EACH_BB_FN (bb, cfun)
      ...
      for (gsi = gsi_start_bb (bb); !gsi_end_p (gsi); gsi_next (&gsi))
	{
	  ...
	  /* We want to mark the block referenced by the inline entry
	     point marker as used.  It would be bad to remove it.
	     There's nothing else in inline entry point markers that
	     could possibly be marked unintentionally.  */
	  if (is_gimple_debug (stmt) && !gimple_debug_inline_entry_p (stmt))
	    continue;
